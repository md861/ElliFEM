...
!Definitions
  ...
  K_W = 30.0D0*PI
    ...
    NGAUSS = 12!10!NGAUSS + CEILING(dsqrt(1.0D0/TOTELS)*K_W*10.0D0/(2.0D0*PI))
    NANGL = 1!9	!Must be replaced to 1 + n_Beta if ENR_type = 1 (ENR2)
    NODEF = NODEL*NANGL
  ...
  !Build marked indices for DBC--------------------------------------Starts
    DBC_pos = 2				!NB: DBC at pos = 1 is always zero Dirichlet 
  ...
  !!Build marked indices for DBC2--------------------------------------Starts
  !  DBC2_pos = 2			 	
  !  TotDBC2NODS = TOTNODS_BC(DBC2_pos)	        !# of DBC2 nodes p-FEM
  !  TotNonDBC2NODS = TOTNOD - TotDBC2NODS	!# of nonDBC2 nodes p-FEM    
  !  allocate(temp_NonDBC2Nodes(TotNonDBC2NODS))
  !  allocate(temp_DBC2Nodes(TotDBC2NODS))
  !  allocate(NonDBC2Nodes(TotNonDBC2NODS*NANGL))
  !  allocate(DBC2Nodes(TotDBC2NODS*NANGL))
  !  call getDBCMarker(MrkdNds_BC,TOTNODS_BC,TOTCOORD,NMARK,&
  !       TOTNOD,temp_NonDBC2Nodes,temp_DBC2Nodes,DBC2_pos)
  !    !write(*,*)'temp_DBC2Nodes = ',temp_DBC2Nodes
  !    !write(*,*)'temp_NonDBC2Nodes = ',temp_NonDBC2Nodes
  !  !update the marked indices for PUFEM coefficients.
  !  call getPUFEM_DBCMarker(temp_NonDBC2Nodes,temp_DBC2Nodes,&
  !                          TotDBC2NODS,TOTNOD,NANGL,&
  !                          NonDBC2Nodes,DBC2Nodes)
  !    !write(*,*)'DBC2Nodes = ',DBC2Nodes
  !    !write(*,*)'NonDBC2Nodes = ',NonDBC2Nodes
  !  !update # of DBC2/NonDBC2 coefficients
  !  TotDBC2NODS = TotDBC2NODS*NANGL
  !  TotNonDBC2NODS = TotNonDBC2NODS*NANGL
  !  !allocate DBC2 Matrices/Vectors for [M],Phi
  !  allocate(InvGM_DBC2(TotDBC2NODS,TotDBC2NODS))
  !  allocate(Phi_DBC2(TotDBC2NODS))
  !  
    !get unmarked nodes
  ...
  !  
  !!Get DBC Mass and inverseMass
  !  call inv(GM(DBC2Nodes,DBC2Nodes),TotDBC2NODS,InvGM_DBC2,TotDBC2NODS)  
  !  !write(*,*)'GM(DBC2Nodes,DBC2Nodes) = ',GM(DBC2Nodes,DBC2Nodes)
